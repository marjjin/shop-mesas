...
api.MapPost("/tables/{id:int}/open", async (int id, RestaurantContext db) => { var table = await db.Tables.FindAsync(id); if (table is null) return Results.NotFound(); table.Status = "occupied"; table.OpenedAt = table.LastConsumptionAt = DateTime.UtcNow; await db.SaveChangesAsync(); return Results.Ok(table); });

...
var configuredConnectionString = builder.Configuration.GetConnectionString("Restaurant")
    ?? "Data Source=gestion-mesas.db";
builder.Services.AddDbContext<RestaurantContext>(options =>
{
    if (IsPostgreSqlConnectionString(configuredConnectionString))
        options.UseNpgsql(ToNpgsqlConnectionString(configuredConnectionString));
    else
...
builder.Services.AddScoped<ICigaretteService, CigaretteService>();
...
using (var scope = app.Services.CreateScope())
{
    ...
    db.Database.ExecuteSqlRaw("""
        ...
        """);
    db.Database.ExecuteSqlRaw("""
        CREATE TABLE IF NOT EXISTS "PendingOrderItems" (
            "Id" integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        ...
        """);
    db.Database.ExecuteSqlRaw("""
        CREATE TABLE IF NOT EXISTS "CigaretteProducts" (
            "Id" integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        ...
        CREATE UNIQUE INDEX IF NOT EXISTS "IX_CigaretteProducts_Name" ON "CigaretteProducts" ("Name");
        CREATE TABLE IF NOT EXISTS "CigarettePurchases" (
            "Id" integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            "CigaretteProductId" integer NOT NULL REFERENCES "CigaretteProducts" ("Id"),
            "Quantity" integer NOT NULL,
            "UnitCost" numeric(12,2) NOT NULL,
            "BusinessDate" date NOT NULL,
            "Shift" character varying(16) NOT NULL,
            "CreatedAt" timestamp with time zone NOT NULL
        );
        CREATE TABLE IF NOT EXISTS "CigaretteShiftCloses" (
            "Id" integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            "BusinessDate" date NOT NULL,
            "Shift" character varying(16) NOT NULL,
            "ClosedAt" timestamp with time zone NOT NULL
        );
        CREATE UNIQUE INDEX IF NOT EXISTS "IX_CigaretteShiftCloses_BusinessDate_Shift" ON "CigaretteShiftCloses" ("BusinessDate", "Shift");
        CREATE TABLE IF NOT EXISTS "CigaretteShiftCloseItems" (
            "Id" integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
            "CigaretteShiftCloseId" integer NOT NULL REFERENCES "CigaretteShiftCloses" ("Id") ON DELETE CASCADE,
            "CigaretteProductId" integer NOT NULL REFERENCES "CigaretteProducts" ("Id"),
...
api.MapGet("/bootstrap", async (RestaurantContext db) => Results.Ok(new
{
    tables = await db.Tables.ToListAsync(),
    products = await db.Products.ToListAsync(),
    categories = await db.Categories.ToListAsync(),
    orders = await db.Orders.ToListAsync(),
    pendingOrders = await db.PendingOrders.OrderBy(order => order.CreatedAt).ToListAsync(),
    pendingOrderItems = await db.PendingOrderItems.OrderBy(item => item.CreatedAt).ToListAsync()
}));
...
api.MapDelete("/categories/{id:int}", async (int id, RestaurantContext db) => { var category = await db.Categories.FindAsync(id); if (category is null) return Results.NotFound(); if (category.Name == CoworkingBilling.CategoryName) return Results.Conflict(); var productIds = await db.Products.Where(product => product.CategoryId == id).Select(product => product.Id).ToListAsync(); db.Orders.RemoveRange(db.Orders.Where(order => productIds.Contains(order.ProductId))); db.PendingOrderItems.RemoveRange(db.PendingOrderItems.Where(item => productIds.Contains(item.ProductId))); db.Products.RemoveRange(db.Products.Where(product => product.CategoryId == id)); db.Categories.Remove(category); await db.SaveChangesAsync(); return Results.NoContent(); });
...
api.MapDelete("/products", async (RestaurantContext db) => { var productIds = await db.Products.Where(product => product.Name != CoworkingBilling.HalfHourProductName && product.Name != CoworkingBilling.HourProductName).Select(product => product.Id).ToListAsync(); db.Orders.RemoveRange(db.Orders.Where(order => productIds.Contains(order.ProductId))); db.PendingOrderItems.RemoveRange(db.PendingOrderItems.Where(item => productIds.Contains(item.ProductId))); db.Products.RemoveRange(db.Products.Where(product => productIds.Contains(product.Id))); await db.SaveChangesAsync(); return Results.NoContent(); });
api.MapDelete("/products/{id:int}", async (int id, RestaurantContext db) => { var product = await db.Products.FindAsync(id); if (product is null) return Results.NotFound(); if (product.Name is CoworkingBilling.HalfHourProductName or CoworkingBilling.HourProductName) return Results.Conflict(); db.Orders.RemoveRange(db.Orders.Where(order => order.ProductId == id)); db.PendingOrderItems.RemoveRange(db.PendingOrderItems.Where(item => item.ProductId == id)); db.Products.Remove(product); await db.SaveChangesAsync(); return Results.NoContent(); });
api.MapPost("/pending-orders", async (CreatePendingOrderRequest request, RestaurantContext db) =>
{
    var customerName = request.CustomerName.Trim();
    if (string.IsNullOrWhiteSpace(customerName) || customerName.Length > 80) return Results.BadRequest();
    var pendingOrder = new PendingOrder { CustomerName = customerName, CreatedAt = DateTime.UtcNow };
    db.PendingOrders.Add(pendingOrder);
    await db.SaveChangesAsync();
    return Results.Created($"/api/pending-orders/{pendingOrder.Id}", pendingOrder);
});
api.MapDelete("/pending-orders/{id:int}", async (int id, RestaurantContext db) =>
{
    var pendingOrder = await db.PendingOrders.FindAsync(id);
    if (pendingOrder is null) return Results.NotFound();
    db.PendingOrders.Remove(pendingOrder);
    await db.SaveChangesAsync();
    return Results.NoContent();
});
api.MapPost("/pending-orders/{id:int}/items", async (int id, AddPendingOrderItemRequest request, RestaurantContext db) =>
{
    if (!await db.PendingOrders.AnyAsync(order => order.Id == id)) return Results.NotFound();
    var product = await db.Products.FindAsync(request.ProductId);
    if (product is null || product.Name is CoworkingBilling.HalfHourProductName or CoworkingBilling.HourProductName)
        return Results.BadRequest();

    var item = await db.PendingOrderItems.FirstOrDefaultAsync(existing => existing.PendingOrderId == id && existing.ProductId == request.ProductId);
    if (item is null)
    {
        item = new PendingOrderItem { PendingOrderId = id, ProductId = request.ProductId, Quantity = Math.Max(1, request.Quantity), CreatedAt = DateTime.UtcNow };
        db.PendingOrderItems.Add(item);
    }
    else
...
api.MapDelete("/pending-order-items/{id:int}", async (int id, RestaurantContext db) =>
{
    var item = await db.PendingOrderItems.FindAsync(id);
    if (item is null) return Results.NotFound();
    db.PendingOrderItems.Remove(item);
    await db.SaveChangesAsync();
    return Results.NoContent();
});
api.MapPost("/pending-orders/{id:int}/assign", async (int id, AssignPendingOrderRequest request, RestaurantContext db, CancellationToken cancellationToken) =>
{
    var assignment = await PendingOrderWorkflow.AssignAsync(db, id, request.TableId, DateTime.UtcNow, cancellationToken);
    return assignment.Status switch
    {
        PendingOrderAssignmentStatus.Success => Results.Ok(new { table = assignment.Table }),
        PendingOrderAssignmentStatus.PendingOrderNotFound => Results.NotFound(),
        PendingOrderAssignmentStatus.TableNotAvailable => Results.Conflict(),
